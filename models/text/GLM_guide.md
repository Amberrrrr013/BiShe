    ...
    ChatCompletionTextRequest:
      ...
      properties:
        model:
          type: string
          description: >-
            调用的普通对话模型代码。`GLM-5` 和 `GLM-5-Turbo` 是最新的旗舰模型系列。`GLM-5` `GLM-4.7`
    ...
    ChatCompletionVisionRequest:
      ...
      properties:
        ...
        temperature:
          type: number
          description: >-
            采样温度，控制输出的随机性和创造性，取值范围为 `[0.0, 1.0]`，限两位小数。对于`GLM-5V-Turbo`默认值为
            `1.0`，`GLM-4.6V`, `GLM-4.5V`系列默认值为 `0.8`，`AutoGLM-Phone`默认值为
            `0.0`，`GLM-4.1v`系列默认值为
        ...
        top_p:
          ...
          description: >-
            核采样（`nucleus sampling`）参数，是`temperature`采样的替代方法，取值范围为 `[0.01,
            1.0]`，限两位小数。对于`GLM-5V-Turbo`默认值为 `0.95`，`GLM-4.6V`, `GLM-4.5V`系列默认值为
    ...
    ChatThinking:
      ...
      properties:
        type:
          type: string
          description: >-
            是否开启思维链(当开启后 `GLM-5`  `GLM-5v-Turbo` `GLM-4.7` `GLM-4.5V`
